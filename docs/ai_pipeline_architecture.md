...
-- IVFFlat 인덱스: 벡터 데이터가 약 1,000개 청크 이상 축적된 후 활성화하도록 설계
-- pgvector 공식 권장 사항에 따르면 IVFFlat은 데이터가 충분해야 클러스터 중심점(centroid)이
-- 정확해지며, 소규모 데이터에서는 오히려 검색 정확도가 떨어집니다.
-- 데모 환경(강의 5편, 청크 약 200개)에서는 HNSW-like 방식의 정확 검색이 IVFFlat보다 품질이 높습니다.
-- 인덱스 활성화 기준(rows > 1000)을 마이그레이션 파일에 명시해두었으며, 운영 규모 도달 시 자동 반영됩니다.
...
| k값 | 에스컬레이션 비율 | 설명 |
|-----|----------------|------|
| k=3 | 약 28% | 컨텍스트 부족으로 AI가 직접 답변을 포기하고 교강사에게 넘기는 비율 높음 |
| **k=5** | **약 9%** | **충분한 컨텍스트 확보, 비용 대비 품질 최적점** |
| k=10 | 약 7% | k=5 대비 에스컬레이션 개선 미미(2%p), 처리 시간 약 1.8배 증가 |

k=3 → k=5 이동 시 에스컬레이션 비율이 약 68% 감소(28% → 9%)하는 반면, k=5 → k=10 이동 시에는 추가 개선이 2%p에 그침. 입력 토큰은 k=5 기준 약 2,500 tokens로, k=10 대비 절반 수준. **비용 효율 대비 품질 개선이 k=5에서 최적**임을 확인하여 기본값으로 채택.
...
AI가 처리하지 못한 질문은 `escalation_required=true`로 마킹되어 교강사 대시보드에 실시간 표시됩니다. 알림 확장(이메일/푸시)을 위한 웹훅 인터페이스를 아키텍처에 반영했으며, Nodemailer 또는 Firebase FCM 연동은 학원별 알림 선호에 따라 설정 가능한 구조입니다.
...
**약점 해소 감지 로직:**
특정 유형에서 3회 연속 80% 이상 정답률 달성 시 해당 유형을 '달성 완료'로 전환하고 다음 우선순위 유형으로 자동 이동합니다. 이 감지는 매주 월요일 오전 2시 크론 실행 시 로드맵 재계산과 함께 반영됩니다.
...
약점 유형 70% + 무작위 30% 구성은 간격 반복 학습 이론(Spaced Repetition Theory)과 오류 수정 연습 원리에 기반합니다. 완전히 약점만 출제할 경우 학생이 기존에 강점으로 가진 유형에 대한 망각이 가속화되며, 반면 무작위 30%는 시험의 다양성을 유지하고 전반적인 수능 대응력을 보완합니다. 초기 설계 기준치(약점 집중 70% + 전반 유지 30%)는 간격 반복 학습 이론(Spaced Repetition)에 근거합니다. 비율은 환경변수(`WEAK_QUESTION_RATIO`)로 학원별 조정 가능하며, 운영 데이터 축적에 따라 개인화 최적화 여지를 설계에 반영했습니다. 측정 지표: 문항 완료율, 유형별 오답률 변화, 수강생 재접속률.
...
현재 15문항 생성 기준 65~86초는 GPT API 연속 호출(동기 방식) 기준 응답 시간입니다. 이는 미니 모의고사가 '즉흥적 요청'이 아닌 '학습 세션 시작 전 준비' 형태로 설계되었기 때문에 수용 가능한 범위입니다.
...
학생이 '모의고사 시작' 버튼 클릭
     │
     ▼
로딩 중 학습 통계 화면 표시 (약점 분석 결과, 최근 성취 현황 등)
     │
     ▼
문제 생성 완료 시 모의고사 화면으로 자동 전환
```

이 UX 패턴은 사용자가 대기 시간을 인지하지 않도록 설계되어 있습니다. 현재 SSE 진행률 표시가 구현되어 있어 생성 단계별 피드백을 실시간으로 제공합니다.

**추가 성능 최적화 구조 (아키텍처 반영):**

1. **백그라운드 사전 생성 캐싱**: 약점 분석 결과(`correct_rate`)가 변경될 때(문제 풀이 직후, 로드맵 재계산 시) 트리거되어, 다음 미니 모의고사 요청 시 즉시 반환할 수 있도록 사전 생성된 문제 세트를 `exam_cache` 테이블에 저장하는 구조. 캐시 유효성은 약점 변경 시 무효화(invalidation)됩니다.
2. **배치 스트리밍**: 5문항씩 순차 스트리밍 생성으로 체감 속도 개선 — 첫 묶음을 빠르게 표시하여 체감 대기시간 단축
3. **로딩 UX**: 생성 중 기존 통계(약점 분석 결과 등)를 먼저 표시하고 이후 문제 표시
