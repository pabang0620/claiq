...
## 데모 시드 데이터 전면 재구성 - 2026-04-13

### 작업 배경
기존 시드 데이터가 페이지별 최소 요건을 충족하지 못해, 공모전 데모 시 화면이 비어 보이거나 통계가 표시되지 않는 문제가 있었다. 모든 페이지가 의미 있게 동작하도록 시드 데이터를 전면 재작성했다.

### AI와 함께 한 것

- 프론트엔드 페이지 전수 조사 → 화면별 필요 데이터 목록화
- API 호출 경로 추적 (page → api client → backend route → repository → table)
- DB 마이그레이션 파일 전수 확인으로 스키마·FK 순서·ENUM 값 파악
- `000_reset.sql` 신규 작성 (FK 역순 TRUNCATE로 멱등성 보장)
- `004_demo_data.sql` 재작성 (정적 마스터 데이터)
- `005_demo_activity.sql` 전면 재작성 (활동 데이터 3000+ 줄)
- SQL 문법 오류 반복 디버깅 (수학 미분 표기 `f'(x)`, 한국어 인용구 내 single quote)

### 핵심 결정 및 근거

1. **000_reset.sql 도입**: 기존에는 ON CONFLICT DO NOTHING만 사용해 오래된 데이터가 남는 문제가 있었다. TRUNCATE → 재삽입 방식으로 전환해 언제든 시드를 재실행해도 동일한 결과를 보장한다.

2. **UUID 그룹 prefix 체계 정비**: `0002=lectures`, `0003=questions`, `0004=roadmaps`, `0005=exams`, `0006=materials`, `0007=qa_sessions`, `0008=qa_messages`로 그룹별 앞자리를 고정해 디버깅 시 레코드 소속을 즉시 파악 가능.

3. **SQL single quote 이스케이프 전략**: 한국어 인용 구문(`'추론'`)과 수학 미분 표기(`f'(x)`)에서 내부 single quote가 SQL 문자열 종결자로 인식되는 문제 → Python 정규식으로 일괄 탐지 후 `''` 이중 처리. PostgreSQL 이스케이프 리터럴 `E'...'` 접두사는 건드리지 않도록 예외 처리.

### 결과

- 시드 파일 6개 전체 실행 성공 (`000` ~ `005`)
- 강의 10개 (국어 4, 수학 3, 영어 3), 문제 37개, 출결 4주치, QA 10건, 모의고사 9개, 뱃지/포인트/로드맵 전 수강생 보유
- 운영자·교강사·수강생 모든 주요 페이지 데이터 유무 검증 완료
- `claiq-seed-generator.md` 에이전트 정의 파일 작성 (재실행 기준 문서화)
