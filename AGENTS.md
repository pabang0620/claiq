...
백엔드 전체를 담당했습니다. OpenAI Whisper STT 연동, pgvector 임베딩 저장, GPT-4o-mini 문제 생성 API, SSE(Server-Sent Events) 실시간 진행상황 전송, RAG Q&A 엔드포인트 등을 구현했습니다.
...
핵심 기능(문제 생성 파이프라인, RAG 검색 정확도, 난이도 분류 로직)에 대해 테스트를 먼저 작성하고 구현하는 TDD 워크플로우를 가이드했습니다. 핵심 비즈니스 로직의 테스트 커버리지 80% 이상을 달성했습니다.
...
### 비용 효율
- security-reviewer와 architect 협업으로 gpt-4o-mini 우선 사용 전략 수립 → **월 AI API 비용 1~2만원 수준** 달성
