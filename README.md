...
| 학부모 | 자녀 학습 현황을 학원에 물어봐야만 앎 | 성취 리포트 자동 생성 + SMS 발송 |
...
③  gpt-4o-mini 수능 유형 매핑 → 강의 내용 → 수능 출제 유형 자동 태깅
④  gpt-4o-mini 문제 자동 생성 → 난이도 A/B/C · 5지선다 · 단답형
...
⑧  gpt-4o-mini RAG 답변 생성 → SSE 스트리밍으로 실시간 반환
         │ AI 처리 불가
         └──→ 교강사 에스컬레이션 알림
```
...
| ③ 수능 유형 매핑 (gpt-4o-mini) | gpt-4o-mini | 입력 2K + 출력 500 tokens | ~$0.0006 |
| ④ 문제 생성 3난이도 (gpt-4o-mini) | gpt-4o-mini | 입력 6K + 출력 3K tokens × 3 | ~$0.0063 |
| **강의 1편 합계** | | | **~$0.19** |

> 비용 대부분은 STT(Whisper-1)에서 발생. gpt-4o-mini 단가: $0.15/$0.60 per M tokens.

### 수강생 세션 비용 (1회 기준)

| 기능 | API | 토큰 | 비용 |
|------|-----|------|------|
| AI Q&A 1회 (RAG) | gpt-4o-mini | 입력 2K + 출력 500 tokens | ~$0.0006 |
| 미니 모의고사 생성 | gpt-4o-mini | 입력 3K + 출력 2K tokens | ~$0.0017 |
| D-day 로드맵 생성 | gpt-4o-mini | 입력 2K + 출력 1K tokens | ~$0.0009 |
...
├─ 교육 커뮤니티·뉴스 70건 수집 (crawl_pain_points.py)
│   └─ 역할별 불만 분류 → 기능 1:1 대응
└─ 공모전 경쟁작 GitHub 10개 병렬 크롤링 (crawl_competitors.py)
...
| LLM | OpenAI gpt-4o-mini (기본) / gpt-4o (환경변수 전환) | 수능 문체 생성 · 비용 효율 최적 |
...
OPENAI_API_KEY=sk-...
OPENAI_MODEL_CHAT=gpt-4o-mini   # gpt-4o로 변경 시 고품질 모드 (비용 약 15배)
JWT_SECRET=...
...
| [docs/adr/ADR-001](./docs/adr/ADR-001-ai-model-selection.md) | AI 모델 선택 근거 (gpt-4o-mini 채택) |
...
| 심사 기준 | CLAIQ 대응 |
|----------|-----------|
| 기술적 완성도 | 5단계 AI 파이프라인 · SSE · pgvector RAG · 실 배포 |
