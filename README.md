...
- **대시보드** - 수강생 수, 이탈위험 수, 출석률, 미발송 리포트 수 한눈에 파악
- **이탈 예측 분석** - 출결 데이터 기반 `at_risk`(3일 미활동) / `inactive`(7일 미활동) 자동 분류, AI 코멘트 생성
- **강의별 통계** - 강의별 출석·문제·정답률 시각화, 과목/기간 필터로 심층 분석
- **회원 관리** - 목록 조회, 전체/교강사/수강생 탭 필터
- **학원 설정** - 학원 정보 수정, 쿠폰 CRUD (percent/fixed 할인, 유효기간 설정)
- **성취 리포트** - 수강생별 성취도 자동 생성, SMS 발송(시뮬레이션), 공개 링크

### 👩‍🏫 교강사 (Teacher)

- **강의 업로드** - 오디오 업로드, Whisper-1 STT → 트랜스크립트 자동 생성, pgvector에 청크 저장
- **AI 문제 생성** - GPT-4o-mini 기반 수능 유형 자동 매핑 + 난이도 A/B/C 문제 자동 생성, SSE로 실시간 진행상황 전달
- **문제 검수** - pending/approved/rejected 탭, 상세 편집 후 승인 또는 반려
- **강의 자료** - PDF/이미지 업로드, 수강생 열람
- **출결 관리** - 날짜 선택 후 출석/결석/지각/공결 일괄 저장
- **QA 에스컬레이션** - AI 미답변 질문 수신, 직접 답변 처리

### 🎒 수강생 (Student)

- **대시보드** - D-day, 스트릭, 포인트, 로드맵 진행률 한눈에 확인
- **오늘의 문제** - approved 문제만 출제, 5지선다/단답형, 즉시 채점 및 해설 제공
- **AI Q&A** - 해당 강사 강의 내용 기반 RAG (pgvector 임베딩 검색), GPT-4o-mini 스트리밍 답변, 세션 관리, 에스컬레이션 가능
- **약점 분석** - 유형별 정답률 차트, 취약 유형 Top 목록 시각화
- **D-day 로드맵** - 수능 역산 주간 학습 계획, 항목 완료 체크, 매주 재계산
- **미니 모의고사** - AI 생성 15문항 (약점 70% + 기타 30%), 20분 타이머, 채점 후 유형별 분석 제공
- **강의 자료** - 강사 업로드 자료 열람
- **포인트** - 잔액·내역·쿠폰 교환
- **뱃지** - 7종 자동 부여 (첫 문제 도전, 7일/30일 연속, 완벽한 풀이, 모의고사 마스터, AI Q&A 탐험가, 로드맵 달성)
...
| 백엔드 | express-engineer | 12개 도메인, AI 모듈 7개 (Whisper/Embedding/QuestionGenerator/TypeMapper/RagQA/RoadmapGenerator/ExamGenerator) |
| 프론트엔드 | react-specialist | 32개 페이지, 9개 Zustand 스토어 (authStore, uiStore, academyStore 등) |
| 품질 | doc-updater | README 및 기술 문서 동기화 |
| 기획 리서치 | - | 수능 학원 시장 분석, 사용자 페르소나 연구 |
| 테스트 | - | 로컬 수동 E2E 테스트 (로그인 → 기능 검증) |
...
| 성취 리포트 (생성·SMS 발송) | ✅ | - | - |
| 강의 녹음 업로드 (SSE 진행상황) | - | ✅ | - |
| AI 문제 자동생성 (Whisper+GPT-4o-mini) | - | ✅ | - |
...
| 수강생 | s1@demo.claiq.kr ~ s5@demo.claiq.kr | demo1234 |
...
| 문서 | 설명 |
|------|------|
| [CLAIQ_심사자_데모가이드.md](./CLAIQ_심사자_데모가이드.md) | 심사자용 10분 탐색 가이드 - 데모 계정, 추천 순서, 핵심 체크포인트 |
...
> **[CLAIQ_심사자_데모가이드.md](./CLAIQ_심사자_데모가이드.md)** — 별도 설치 없이 10분 안에 전체 기능 체험 가능한 단계별 탐색 순서

로그인 화면 하단의 **"데모 계정으로 빠른 시작"** 버튼 클릭만으로 운영자/교강사/수강생 역할 즉시 전환 가능합니다.
